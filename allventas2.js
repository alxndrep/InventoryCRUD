...
$(document).ready(function(){
    $('.selectpicker').selectpicker('val','allventas');
...
function format (d, info ) {
    ...
    return '<dl class="row">'+
                ...
                `<dd class="col-sm-2"><button estadoventa="${d.estado}" totalventa="${d.total}" ventaid="${d.venta_id}" class="btn btn-outline-primary btn-sm btnVerProductos">Ver Detalle</button></dd>`+
                '<dd class="col-sm-10"></dd>'+
        ...
        "columns":[
            ...
            {"data": "total",className:"ventaclientetotal"},
            {
                ...
                "render": function(data){
                    if(data == 1){
                        return '<i class="estadoVentaTrue"></i>'
                    }else{
                        return '<i class="estadoVentaFalse"></i>'
                    }
                }
            }
        ]
...
$(document).on("click",".btnActualizarEstadoVenta", function(e){
    opcion = 13;
    venta_id = $(this).attr('ventaid');
    estadoVenta = $(this).attr('estadoventa');
    ...
    $.ajax({
        ...
        type: "POST",
        datatype:"json",    
        data:  {opcion:opcion, venta_id:venta_id, estado:estado},    
        success: function(data) {
            tablaBusqueda.ajax.reload(null, false);
            $('#pushNotificationToastBody').html(`Venta con Folio: ${venta_id} finalizada exitosamente.`);
         },
    });	
    $('#modalProductosVentas').modal('hide');
...
$(document).on("click",".btnBorrarVenta", function(e){
    opcion = 3;
    ...
    if(respuesta){    
        $.ajax({
            ...
            success: function(data)
            {
                $('#pushNotificationToastBody').html(`Venta con Folio: ${venta_id} ha sido borrada del sistema.`);
                $('#pushNotificationToast').toast('show');
                tablaBusqueda.ajax.reload(null, false);
            }
        });
...
$(document).on("click",".btnVerProductos", function(){
    ...
    estadoVenta = parseInt(($(this).attr('estadoventa')).replace(/[.,\s]/g, ''));

    $(".btnActualizarEstadoVenta").attr("ventaid",venta_id);
    $(".btnActualizarEstadoVenta").attr("estadoventa",estadoVenta);
        ...
        success: function(data)
        {
            ...
            productos +=    '</tbody><tfoot><tr>'+
                            ...
                            '<tr>'+
                                ...
                                '<td id="ventaIVA19">$'+ Math.round(((total_venta/1.19)*0.19)).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.") +'</td>'+
                            '</tr>'+
                            '<tr>'+
                                '<td></td>'+
                                '<td></td>'+
