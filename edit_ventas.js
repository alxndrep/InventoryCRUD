...
$(document).ready(function() {
    ...
    ({
        ...
        success: function(data){
            ...
            $('#nvcliente').val(data[0]['razon_social']);
            $('#nvrut').val(data[0]['rut']);
            $('#nvgiro').val(data[0]['giro']);
            $('#nvdireccion').val(data[0]['direccion']);
            $('#nvcomuna').val(data[0]['comuna']);
            $('#nvfolio').val(data[0]['nro_folio']);
            $('#nvvendedor').val(data[0]['vendedor']).change();
            $('#nvcontacto').val(data[0]['contacto']);
            $('#nvfecha').val(data[0]['fecha']);
            $('#nvfolio').attr("isborrador", data[0]['isborrador']);
            console.log($('#nvfolio').attr("isborrador"));
        }
    });
            ...
            for(var i = 0 ; i<data.length ; i++){
                $(`
                    <tr id="${data[i]['codigo']}">
                        ...
                        <td><input class="form-control" type="text" value="${data[i]['codigo']}" readonly></td>
                        <td><input class="form-control productoNombre" codigo="${data[i]['codigo']}" type="text" value="${data[i]['nombre']}" maxlength=250></td>
                        <td><input class="form-control productoCantidad" codigo="${data[i]['codigo']}" type="number" style="text-align: center;" min="0" max="10000" value="${data[i]['cant']}"></td>
                        <td><input class="form-control productoPrecio" codigo="${data[i]['codigo']}" type="number" style="text-align: right;" value="${parseFloat(data[i]['precio'].replace(/,/g, ''))}" data-toggle="tooltip" data-placement="top" title="Precio con IVA: $${Math.ceil(parseFloat(data[i]['precio'].replace(/,/g, ''))*1.19)}"></td>
                        <td><input class="form-control productoTotal" codigo="${data[i]['codigo']}" type="number" style="text-align: right;" value="${parseFloat(data[i]['total'].replace(/,/g, ''))}" readonly></td>
            ...
            if (typeof $(trproducto).attr('id') !== 'undefined') {
                ...
                $(`
                    <tr id="${codigo}">
                        ...
                        <td><input class="form-control productoNombre" codigo="${codigo}" type="text" value="${nombre}" maxlength=250></td>
...
function guardarVentaDB(){
    ...
    ({
        url: 'productosCRUD/bd/ventaproducto.php',
        data: {opcion:opcion, nvproductos:nvproductos, venta_id:venta_id, fecha:fecha, vendedor:vendedor, notaventa:notaventa, rut:rut},
        cache: false,
        type: "POST",
        success: function(data){
...
function printBorrador(){
  ...
  "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
    ...
    if($('#nvfolio').val() == ''){
        $('#borradorFolio').text(0);
    }else{
        $('#borradorFolio').text($('#nvfolio').val());
    }
